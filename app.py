...
with st.sidebar:
    st.subheader("🌐 坐标系")
    st.selectbox("输入坐标系", ["WGS-84", "GCJ-02 (高德/百度)"])
    st.markdown("---")
    st.subheader("📊 系统状态")
    st.success("✅ 系统正常")
...
with tab1:
    ...
    with col_map:
        ...
        map_out = st_folium(m, width=650, height=450, key="main_map")
...
st.markdown("---")
st.caption("操作说明：绘制障碍物多边形→设置建筑高度→生成左/右/最优3套航线，算法逐栋绕行连续建筑群，航线全程处于蓝色安全缓冲区外侧，不会穿墙")
